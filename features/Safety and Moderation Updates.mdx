...
Playlab's moderation system automatically detects and hides potentially inappropriate AI messages from being shown to users. The team is actively refining how moderation works to reduce disruptive false positives (messages that shouldn't have been flagged) and improve clarity.
...
- **Messages flagged for reasons related to self-harm are always sent to org admins as urgent notifications in real time** (see below)

### Self-Harm Alerts

Messages flagged for reasons related to self-harm bypass the standard digest and are sent to org admins as urgent, real-time notifications. This ensures that the people responsible for student safety are immediately aware when a student may be in distress.
...
<Warning>
Self-harm alerts cannot be turned off. They are always sent in real time to your organization's designated safety notification recipients.
</Warning>
...
## <Icon icon="user-shield" size="24" className="inline-block align-text-bottom" /> Safety Notification Designation
